...
def train_none(disc, disc_loss_fn, disc_opt, train_dataloader, test_dataloader, device, n_epochs, suppress_output, save_dir):
    from train_single_model import train_epoch, eval_epoch
    current_epoch = 0
    def run_epoch():
...
def run_trial(
    ...
    aux_disc_loss_constructor=None, aux_disc_kwargs={}, # loss function for above
    ...
    disc         = construct(disc_constructor, **disc_kwargs)
    disc_loss_fn = construct(disc_loss_constructor, **disc_loss_kwargs)
    disc_opt     = construct(disc_opt_constructor, disc.parameters(), **disc_opt_kwargs)
    gen          = construct(gen_constructor, **gen_kwargs)
    gen_loss_fn  = construct(gen_loss_constructor, **gen_loss_kwargs)
    gen_opt      = construct(gen_opt_constructor, gen.parameters(), **gen_opt_kwargs)
    ...
    train_dataloader = construct(DataLoader, shuffle=True, **dataloader_kwargs)
    test_dataloader  = construct(DataLoader, shuffle=False, **dataloader_kwargs)
    ...
    if not suppress_output:
        ...
        print('Gen loss fn: {}'.format(gen_loss_gn))
